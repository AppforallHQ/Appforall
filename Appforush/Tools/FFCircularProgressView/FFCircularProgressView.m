...
//    _lineWidth = fmaxf(self.frame.size.width * 0.025, 1.f);
    _lineWidth = fmaxf(realSize * 0.025, 1.f);
...
//    _iconLayer.frame = self.bounds;
    _progressBackgroundLayer.frame = CGRectMake(realPosition, realPosition, realSize, realSize);
    _progressLayer.frame = CGRectMake(realPosition, realPosition, realSize, realSize);
    _iconLayer.frame = CGRectMake(realPosition, realPosition, realSize, realSize);

//    CGPoint center = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2);
//    CGFloat radius = (self.bounds.size.width - _lineWidth)/2;
    CGPoint center = CGPointMake(realSize/2, realSize/2);
    CGFloat radius = (realSize - _lineWidth)/2;
    // Draw background
...
//    radius = (self.bounds.size.width - _lineWidth*3) / 2.0;
    radius = (realSize - _lineWidth*3) / 2.0;
...
//    CGFloat radius = MIN(self.frame.size.width, self.frame.size.height)/2;
    CGFloat radius = MIN(realSize, realSize)/2;
...
//    CGFloat sideSize = self.bounds.size.width * ratio;
    CGFloat radius = (realSize)/2;
    CGFloat ratio = kStopSizeRatio;
    CGFloat sideSize = realSize * ratio;
...
- (void) drawPause {
    ...
    UIBezierPath *pausePath1 = [UIBezierPath bezierPath];
    [pausePath1 moveToPoint:CGPointMake(0, 0)];
    [pausePath1 addLineToPoint:CGPointMake((sideSize/2) - 1, 0.0)];
    [pausePath1 addLineToPoint:CGPointMake((sideSize/2) - 1, sideSize)];
    [pausePath1 addLineToPoint:CGPointMake(0.0, sideSize)];
    [pausePath1 closePath];
    
    [pausePath1 moveToPoint:CGPointMake((sideSize/2) + 1, 0)];
    [pausePath1 addLineToPoint:CGPointMake(sideSize, 0.0)];
    [pausePath1 addLineToPoint:CGPointMake(sideSize, sideSize)];
    [pausePath1 addLineToPoint:CGPointMake((sideSize/2) + 1, sideSize)];
    [pausePath1 closePath];
    
    // ...and move it into the right place.
    [pausePath1 applyTransform:CGAffineTransformMakeTranslation(radius * (1-ratio), radius* (1-ratio))];
    
    [_iconLayer setPath:pausePath1.CGPath];
    [_iconLayer setStrokeColor:_progressLayer.strokeColor];
    [_iconLayer setFillColor:self.tintColor.CGColor];
}
...
//    CGFloat segmentSize = self.bounds.size.width * ratio;
    CGFloat radius = (realSize)/2;
    CGFloat ratio = kArrowSizeRatio;
    CGFloat segmentSize = realSize * ratio;
